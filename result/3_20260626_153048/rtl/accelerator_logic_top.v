...
module accelerator_logic_top #(
    ...
    input  wire                       pulse_en,
    input  wire        [63:0]         a_sram_rdata,
    input  wire        [63:0]         b_sram_rdata,
    ...
    generate
        for (idx = 0; idx < ROWS; idx = idx + 1) begin : gen_a_core_in
            assign a_core_in[idx] = $signed(a_sram_rdata[(7-idx)*8 +: 8]);
        end

        for (idx = 0; idx < COLS; idx = idx + 1) begin : gen_b_core_in
            assign b_core_in[idx] = $signed(b_sram_rdata[(7-idx)*8 +: 8]);
        end
    endgenerate
