...
module accelerator_top (
    ...
    always @* begin
        ...
        if (state == S_LOAD_A_SRAM_WRITE) begin
            a_sram_cs_n[0]  = 1'b0;
            a_sram_we_n[0]  = 1'b0;
            a_sram_addr[0]  = {load_k[5:0], load_lane};
            a_sram_wdata[0] = {
                get_byte(a_word_data[0], load_lane),
                get_byte(a_word_data[1], load_lane),
                get_byte(a_word_data[2], load_lane),
                get_byte(a_word_data[3], load_lane)
            };
            ...
            for (idx = 2; idx < B_BANKS; idx = idx + 1) begin
                b_sram_cs_n[idx]  = 1'b0;
                b_sram_we_n[idx]  = 1'b0;
                b_sram_addr[idx]  = load_k;
            end
            b_sram_wdata[2] = mem_data[63:32];
            b_sram_wdata[3] = mem_data[31:0];
        end else if ((state == S_COMPUTE_ADDR) || (state == S_COMPUTE_PULSE)) begin
            for (idx = 0; idx < A_BANKS; idx = idx + 1) begin
                a_sram_cs_n[idx] = 1'b0;
                a_sram_we_n[idx] = 1'b1;
                a_sram_addr[idx] = ((state == S_COMPUTE_PULSE) && (compute_k != 9'd511)) ? (compute_k + 9'd1) : compute_k;
            end
            for (idx = 0; idx < B_BANKS; idx = idx + 1) begin
                b_sram_cs_n[idx] = 1'b0;
                b_sram_we_n[idx] = 1'b1;
