...
module testbench_top;
    ...
    localparam real T = 2.0;

    reg clk = 1'b0;
    reg rst_n;
    reg clear_acc;
    reg pulse_en;
    reg [31:0] a_sram_rdata;
    reg [31:0] b_sram_rdata0;
    reg [31:0] b_sram_rdata1;
    reg [31:0] b_sram_rdata2;
    reg [31:0] b_sram_rdata3;
`ifdef GATE
...
`ifdef GATE
    accelerator_logic_top u_dut (
        ...
        .b_sram_rdata0(b_sram_rdata0),
        .b_sram_rdata1(b_sram_rdata1),
        .b_sram_rdata2(b_sram_rdata2),
        .b_sram_rdata3(b_sram_rdata3),
        .c_out(c_out_flat)
    );
`else
    ...
    task drive_inputs;
        ...
        begin
            for (row = 0; row < ROWS; row = row + 1) begin
                ...
                a_sram_rdata[(3-row)*8 +: 8] = a_tmp[7:0];
            end
            for (col = 0; col < COLS; col = col + 1) begin
                b_tmp = b_value(col, k_i);
                if (col < 4) begin
                    b_sram_rdata0[(3-col)*8 +: 8] = b_tmp[7:0];
                end else if (col < 8) begin
                    b_sram_rdata1[(7-col)*8 +: 8] = b_tmp[7:0];
                end else if (col < 12) begin
                    b_sram_rdata2[(11-col)*8 +: 8] = b_tmp[7:0];
                end else begin
                    b_sram_rdata3[(15-col)*8 +: 8] = b_tmp[7:0];
                end
            end
        end
    endtask
