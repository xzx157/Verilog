# 快照 1：20260624_235307

本目录是 Lab8 第 1 版实验结果快照，用于保留当时的代码、输入输出数据、综合报告和验证记录。
...
- `ppa_logic_check`、`post_tb_syntax_check`：早期检查时生成的辅助检查文件。
...
- `pdk/`：该快照保留的 NanGate PDK 文件。
- `__pycache__/`：Python 运行 checker 时生成的缓存目录。