...
module accelerator_top (
    ...
    generate
        ...
        for (bank = 0; bank < TILE_ROWS; bank = bank + 1) begin : gen_a_core_in
            assign a_core_in[bank] = $signed(a_sram_rdata[0][(7-bank)*8 +: 8]);
        end

        for (bank = 0; bank < TILE_COLS; bank = bank + 1) begin : gen_b_core_in
            assign b_core_in[bank] = $signed(b_sram_rdata[0][(7-bank)*8 +: 8]);
        end
    endgenerate
    ...
    always @* begin
        for (idx = 0; idx < A_BANKS; idx = idx + 1) begin
            ...
            a_sram_wdata[idx] = 64'd0;
        end
        for (idx = 0; idx < B_BANKS; idx = idx + 1) begin
            b_sram_cs_n[idx]  = 1'b1;
            b_sram_we_n[idx]  = 1'b1;
            b_sram_addr[idx]  = compute_k;
            b_sram_wdata[idx] = 64'd0;
        end

        if ((state == S_LOAD_A_WRITE) && (load_row == 4'd7)) begin
            a_sram_cs_n[0]  = 1'b0;
            a_sram_we_n[0]  = 1'b0;
            a_sram_addr[0]  = load_k;
            a_sram_wdata[0] = set_byte(a_pack_data, load_row[2:0], get_byte(mem_data, load_k[2:0]));
        end else if (state == S_LOAD_B0_WRITE) begin
            b_sram_cs_n[0]  = 1'b0;
            b_sram_we_n[0]  = 1'b0;
            b_sram_addr[0]  = load_k;
            b_sram_wdata[0] = mem_data;
        ...
        if (comp_enb) begin
            ...
            case (state)
                ...
                S_LOAD_A_WRITE: begin
                    a_pack_data <= set_byte(a_pack_data, load_row[2:0], get_byte(mem_data, load_k[2:0]));
                    if (load_row == 4'd7) begin
                        load_row <= 4'd0;
                        a_pack_data <= 64'd0;
