...
module accelerator_top (
    ...
    localparam integer TILE_COLS   = 12;
    localparam integer B_BANKS     = 3;
    ...
    localparam [4:0] S_LOAD_A_SRAM_WRITE = 5'd15;
    ...
    generate
        ...
        for (bank = 0; bank < TILE_ROWS; bank = bank + 1) begin : gen_a_core_in
            assign a_core_in[bank] = get_byte(a_word_data[bank], compute_k[2:0]);
        end
    ...
    always @* begin
        ...
        if (state == S_LOAD_B0_WRITE) begin
            b_sram_cs_n[0]  = 1'b0;
            b_sram_we_n[0]  = 1'b0;
            b_sram_addr[0]  = load_k;
            b_sram_wdata[0] = col_base[2] ? mem_data[31:0] : mem_data[63:32];

            if (!col_base[2]) begin
                b_sram_cs_n[1]  = 1'b0;
                b_sram_we_n[1]  = 1'b0;
                b_sram_addr[1]  = load_k;
            ...
            if (col_base[2]) begin
                ...
                b_sram_we_n[1]  = 1'b0;
                b_sram_addr[1]  = load_k;
                b_sram_wdata[1] = mem_data[63:32];
            end

            b_sram_cs_n[2]  = 1'b0;
            b_sram_we_n[2]  = 1'b0;
            b_sram_addr[2]  = load_k;
            b_sram_wdata[2] = col_base[2] ? mem_data[31:0] : mem_data[63:32];
        end else if ((state == S_COMPUTE_ADDR) || (state == S_COMPUTE_PULSE)) begin
            for (idx = 0; idx < B_BANKS; idx = idx + 1) begin
                b_sram_cs_n[idx] = 1'b0;
                b_sram_we_n[idx] = 1'b1;
                b_sram_addr[idx] = ((state == S_COMPUTE_PULSE) && (compute_k != 9'd511)) ? (compute_k + 9'd1) : compute_k;
            end
        end
    end
            ...
            case (state)
                ...
                S_WRITE_SETUP: begin
                    if ((row_base + {6'd0, write_row}) >= MATRIX_SIZE) begin
                        ...
                        res_addr <= result_word_addr(row_base + {6'd0, write_row}, col_base[8:1] + {5'd0, write_word[2:0]});
                        res_data <= {
                            to_int32(c_mat[write_row][{write_word[2:0], 1'b0}]),
                            to_int32(c_mat[write_row][{write_word[2:0], 1'b0} + 4'd1])
                        };
                        mem_write_enb <= 1'b0;
                        state         <= S_WRITE_HOLD;
                    end
                ...
                S_WRITE_HOLD: begin
                    mem_write_enb <= 1'b0;
                    if (write_word != (col_base == 10'd504 ? 4'd3 : 4'd5)) begin
